...
import { EventFinanceValues, eventFinanceSchema } from '@/schema/finance'
...
const CreateFinance = ({ id }: { id: string }) => {
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const { data, refetch } = useGetEventById(id) // Ensure refetch is available
  ...
  const { fieldArrays, handleAppend, handleRemove, form } =
    useDynamicForm<EventFinanceValues>({
  ...
  const createBudgetMatrix = useMutation({
    ...
    onSuccess: () => {
      ...
      refetch() // Refetch data instead of reloading the page
    },
    onError: (error: any) => {
      console.log(error)
      toast({
  ...
  return (
    <div className="relative ml-auto">
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <DialogContent className="lg:max-w-screen-xl max-w-[725px] overflow-auto md:overflow-y-hidden">
          <DialogHeader>
            <DialogTitle>Budget Matrix</DialogTitle>
          </DialogHeader>
          <div className="relative lg:pt-8 z-10">
            <FormProvider {...form}>
