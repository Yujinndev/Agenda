...
import GroupList from '@/components/event/GroupList'
import { GROUP_VISIBILITY } from '@/constants/choices'
  ...
  return (
    <section className="relative w-full py-4 lg:px-0 gap-4 lg:pt-10">
      <div className="bg-green-900 lg:px-8 p-4 lg:pb-4 rounded-md">
        <div className="pb-6 md:w-2/3 pt-2 lg:w-3/4">
          <h1 className="text-3xl text-white">
            Check out your groups regularly——don't miss the fun!
          </h1>
        </div>
        ...
        <div className="flex flex-col items-center justify-between gap-2 md:flex-row overflow-hidden">
          <div className="flex items-center justify-around gap-2 rounded-xl border bg-white overflow-hidden p-2 md:flex-none flex-wrap">
            {GROUP_VISIBILITY.map((el, idx) => {
              const groupCategoriesLength =
                isSuccess &&
                groups.filter(
                  (group: UserGroup) => group.visibility == el.value
                ).length
        ...
        {isSuccess && (
          <GroupList
            groups={groups.filter(
              (group: UserGroup) => group.visibility == selectedFilter.value
            )}
